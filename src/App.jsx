  ...
  useEffect(() => {
    ...
    ws.current.onmessage = (event) => {
      try {
        ...
        // CASE 1: Standard Telemetry
        if (data.type === 'metric') {
            setCpuLoad(Math.round(data.cpu));
            // NOTE: We REMOVED the client-side scaling logic here.
            // The Server decides now.
        }
        
        // CASE 2: Text Logs from Server
        if (data.type === 'log') {
            addLog(data.message);
        }

        // CASE 3: Action Command (The AI spoke!)
        if (data.type === 'action') {
            setPodCount(data.replicas);
            addLog(`[Auto-Scale] 🤖 Scaled to ${data.replicas} replicas. Reason: ${data.reason}`);
            playVoice(`Automatic intervention. ${data.reason}.`);
        }
